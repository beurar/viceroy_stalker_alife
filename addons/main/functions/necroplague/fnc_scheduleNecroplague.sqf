...
_minDelay = ["VSA_necroMinDelay", _minDelay] call FUNC(getSetting);
_maxDelay = ["VSA_necroMaxDelay", _maxDelay] call FUNC(getSetting);
private _zPerHorde = ["VSA_necroZombies",5] call FUNC(getSetting);
private _hordeCount = ["VSA_necroHordes",2] call FUNC(getSetting);
...
[_minDelay,_maxDelay,_zPerHorde,_hordeCount] spawn {
    ...
    while {true} do {
        if (time >= _next) then {
            [_z,_count] call FUNC(triggerNecroplague);
