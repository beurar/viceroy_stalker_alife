...
if (_count < 0) then { _count = ["VSA_chemicalZoneCount", 2] call FUNC(getSetting); };
...
for "_i" from 0 to (_count - 1) do {
    ...
    if (_debug && {isServer}) then {
        ...
        private _m = [_name, _pos, "ICON", "mil_warning", VIC_colorGasYellow] call FUNC(createGlobalMarker);
    ...
    for "_j" from 1 to _clusterSize do {
        ...
        _zonePos = [_zonePos] call FUNC(findLandPosition);
        if !(isNil {_zonePos} || {_zonePos isEqualTo []}) then {
            [_zonePos, _zoneRadius, _duration] call FUNC(spawnChemicalZone);
        };
    };
};
