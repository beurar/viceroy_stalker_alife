...
if (_count < 0) then { _count = ["VSA_chemicalZoneCount", 2] call viceroy_stalker_alife_cba_fnc_getSetting; };
...
for "_i" from 0 to (_count - 1) do {
    ...
    if (_debug && {isServer}) then {
        ...
        private _m = [_name, _pos, "ICON", "mil_warning", VIC_colorGasYellow] call viceroy_stalker_alife_markers_fnc_createGlobalMarker;
    ...
    for "_j" from 1 to _clusterSize do {
        ...
        _zonePos = [_zonePos] call viceroy_stalker_alife_core_fnc_findLandPosition;
        if !(isNil {_zonePos} || {_zonePos isEqualTo []}) then {
            [_zonePos, _zoneRadius, _duration] call viceroy_stalker_alife_chemical_fnc_spawnChemicalZone;
        };
    };
};
