...
if (_count < 0) then {
    ...
    private _dens = ["VSA_anomalyDensity_Fruitpunch",100] call FUNC(getSetting);
...
private _marker = [_markerName, _site, "ELLIPSE", "", VIC_colorFruitGreen, 1, format ["Fruitpunch %1m", _size]] call FUNC(createGlobalMarker);
_marker setMarkerSizeLocal [_size,_size];
_marker setMarkerBrush "Border";
STALKER_anomalyMarkers pushBack _marker;

// Surround fruitpunch anomalies with a chemical zone
if (["VSA_enableChemicalZones", true] call FUNC(getSetting)) then {
    private _zoneRad = ["VSA_chemicalZoneRadius", 50] call FUNC(getSetting);
    [_site, _zoneRad] call FUNC(spawnChemicalZone);
};
