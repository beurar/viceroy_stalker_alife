...
if (_count < 0) then {
    ...
    private _dens = ["VSA_anomalyDensity_Springboard",100] call FUNC(getSetting);
...
private _marker = [_markerName, _site, "ELLIPSE", "", VIC_colorSpringYellow, 1, format ["Springboard %1m", _size]] call FUNC(createGlobalMarker);
