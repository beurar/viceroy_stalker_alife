...
if (_count < 0) then {
    ...
    private _dens = ["VSA_anomalyDensity_Launchpad",100] call FUNC(getSetting);
...
private _marker = [_markerName, _site, "ELLIPSE", "", VIC_colorTeleport, 1, format ["Launchpad %1m", _size]] call FUNC(createGlobalMarker);
