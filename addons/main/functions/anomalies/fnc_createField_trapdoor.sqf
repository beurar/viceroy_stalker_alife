...
if (_count < 0) then {
    ...
    private _dens = ["VSA_anomalyDensity_Trapdoor",100] call FUNC(getSetting);
...
private _marker = [_markerName, _site, "ELLIPSE", "", VIC_colorTrapdoorTurq, 1, format ["Trapdoor %1m", _size]] call FUNC(createGlobalMarker);
